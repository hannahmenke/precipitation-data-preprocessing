...
- `--output-dir DIR` (default: timestamped folder like 'results_YYYYMMDD_HHMMSS'): Directory where all outputs (models, plots, reports) will be saved. If not specified, a new timestamped folder is created for each run.
...
- Validation data: Excel files in `data_for_classification/` directory (e.g., val3.xlsx, val6.xlsx, 3mM-5_finalData.xlsx, etc.).

The data should contain features like Area, Major, Eccentricity, etc., and a 'type' column for labels (classes 1-4).

## Pipeline Steps
1. **Setup Output Directory**: Creates a timestamped results folder (or uses the specified one) with subdirectories for models, plots, reports, logs, and data.

2. **Data Discovery**: Scans training and validation directories for Excel files, validates them, and reports sample counts, classes, and columns.
...
3. **Feature Engineering** (via `improve_features.py`):
   ...
   - Saves improved datasets: `train_improved.xlsx` and `validation_data_improved.xlsx`.
...
4. **Enhanced Model Training** (via `improve_model_with_clustering.py`):
   ...
   - Saves the enhanced model to `models/enhanced_ensemble_model.joblib`.

5. **Validation Testing & Visualization**:
   - Tests the enhanced model on **all validation datasets** found in `data_for_classification/` (auto-discovered, not hardcoded).
   - Computes metrics: accuracy, precision, recall, F1, confidence statistics.
   - Generates predictions and saves to Excel files.
   - **Creates a comprehensive validation overview plot** (`enhanced_model_validation_results.png`) that includes ALL validation datasets (not just val3/val6/combined). The plot now features:
   ...
   - **Creates individual plots** for each validation dataset (e.g., `enhanced_model_3mM-5_finalData_results.png`), each including a confusion matrix, confidence distribution, and class-wise performance.
   - **Plots are only saved to file, not displayed in the viewer** (for batch/automated use).
   - Also creates train/test performance plots (via `create_train_test_plots.py`).
...
- Comparison with base model on validation set.
...
- **plots/**: 
  ...
  - **Individual validation plots** (e.g., `enhanced_model_3mM-5_finalData_results.png`, `enhanced_model_val3_results.png`, etc.):
    - Each includes a confusion matrix, confidence distribution, and class-wise performance for that dataset
